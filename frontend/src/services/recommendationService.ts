...
export class RecommendationService {
  ...
  private generateAdvancedRecommendation(
    ...
    // Simplified recommendation logic - clear UP/DOWN/HOLD system
    let action = 'HOLD';
    let stars = 3;
    ...
    if (weightedScore >= 25) {
      action = 'UP';
      actionDetail = weightedScore >= 60 ? 'Strong upward trend expected' : 
                     weightedScore >= 45 ? 'Moderate upward trend expected' : 
                     'Mild upward trend expected';
      stars = weightedScore >= 60 ? 5 : weightedScore >= 45 ? 4 : 3;
      ...
      action = 'DOWN';
      actionDetail = weightedScore <= -60 ? 'Strong downward trend expected' : 
                     weightedScore <= -45 ? 'Moderate downward trend expected' : 
                     'Mild downward trend expected';
      stars = weightedScore <= -60 ? 1 : weightedScore <= -45 ? 2 : 2;
      priceTarget = currentPrice * (1 + Math.max(-0.25, weightedScore / 100 * 0.25));
      riskLevel = riskScore > 60 ? 'HIGH' : 'MEDIUM';
    } else {
      action = 'HOLD';
      actionDetail = 'Sideways movement expected - wait for clearer signals';
      stars = 3;
