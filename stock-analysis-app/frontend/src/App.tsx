...
// import InteractiveChart from './components/InteractiveChart'; // Temporarily disabled while fixing
...
function App() {
  ...
  const handleLayoutChange = (layout: 'single' | 'grid' | 'tabs') => {
    ...
    console.log(`🎛️ Dashboard layout changed to: ${layout}`);
  };
  ...
  const analyzeStock = async (symbol?: string) => {
    ...
    try {
      ...
      console.log(`🤖 Running ML pattern recognition analysis for ${targetSymbol}...`);
      try {
        ...
        console.log(`📊 Detected ${detectedPatterns.length} chart patterns`);
        ...
        if (chartDataData.technicalData) {
          ...
          console.log(`💭 Sentiment analysis: ${sentimentAnalysis.overall} (${sentimentAnalysis.strength})`);
          ...
          console.log(`🎯 Ensemble prediction: ${prediction.direction} (${(prediction.confidence * 100).toFixed(1)}% confidence)`);
        }
      ...
      console.log(`📊 Running risk analytics for ${targetSymbol}...`);
      try {
  ...
  const setupRealTimeUpdates = async (symbol: string) => {
    ...
    const unsubscribe = realTimeDataService.subscribe(symbol, (update: RealTimeUpdate) => {
      console.log(`🔴 Real-time update for ${symbol}:`, update);
    ...
    try {
      const unsubscribeTechnical = await realTimeTechnicalService.subscribeToTechnicalUpdates(
        ...
        (technicalData: RealTimeTechnicalData, alerts: TechnicalAlerts) => {
          console.log(`📊 Real-time technical update for ${symbol}:`, technicalData.indicatorUpdates);
          ...
          if (alertCount > 0) {
            console.log(`🚨 ${alertCount} technical alerts triggered for ${symbol}`);
          }
        }
      );

      console.log(`📊 Successfully subscribed to real-time technical indicators for ${symbol}`);
      ...
      console.warn('Failed to setup real-time technical indicators (non-critical):', error);
                  ...
                  {chartData ? (
                    <div style={{ marginTop: '16px' }}>
                      {chartType === 'interactive' ? (
                        // Temporary fallback to AdvancedStockChart while fixing InteractiveChart
                        <div>
                          <div style={{ 
                            padding: '8px 12px', 
                            background: '#fff3cd', 
                            ...
                            🚧 Interactive mode with drawing tools coming soon. Using advanced chart for now.
                          </div>
                          <AdvancedStockChart 
                            chartData={chartData} 
                        ...
                        <StockChart chartData={chartData} height={isMobileView ? 300 : 400} />
                      )}
                    </div>
                  ) : (
                    <div style={{ 
